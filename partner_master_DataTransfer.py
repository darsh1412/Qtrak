...
with SSHTunnelForwarder(
    ...
    try:
        ...
        # Execute a SELECT query to fetch data from the first_register table in Finqy database
        finqy_cursor.execute("SELECT id, companyname, location, rname, pincode, refercode, gst_no, bank_account_name, account_no, bank_name, aadharno, commission, rcontact, email, panno, modified_date, bank_ifsc, city, state, payout, account_type, account_branch, partner_type, product, office_add FROM first_register;")
        ...
        # Iterate through the result set and insert data into the partner_master table in E2E Prod database
        for row in result_set:
            print(f"Inserting data for ID: {row[0]}")

            columns = [col[0] for col in finqy_cursor.description]

            insert_query = f"INSERT INTO partner_master ({', '.join(columns)}) VALUES ({', '.join(['%s'] * len(columns))}) ON DUPLICATE KEY UPDATE {', '.join([f'{col}=VALUES({col})' for col in columns])}"
